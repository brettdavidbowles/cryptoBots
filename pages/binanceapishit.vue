...
export default {
  ...
  methods: {
    ...
    //   const allBitCoinOrders = await this.$axios.$get(`/api/v3/allOrders?symbol=BTCUSDT&timestamp=${this.timestamp}&signature=${this.encryptedkey}`)
    //   this.allBitCoinOrders = allBitCoinOrders
    // },
    async fetchRecentTrades() {
      this.createHmac('BTCUSD')
      const trades = await this.$axios.$get(`/api/v3/allOrders?symbol=BTCUSD&timestamp=${this.timestamp}&recvWindow=5000&signature=${this.encryptedkey}`)
      this.trades = trades
    },
